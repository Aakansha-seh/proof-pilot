...
- **Competitive Intelligence** — source-backed, time-aware competitor research
  (last ~24 months) with a comparison table, Market Activity Signal + Product
  Overlap bars, a Momentum vs Differentiation map, competitor timeline, whitespace
  hypotheses, a "What must you prove to win?" panel, and a claim stress-test that
  feeds back into the Claim Map. The parallel analysis + synthesis stage runs on
  the **AMD Inference Mode** path with real measured timing.
